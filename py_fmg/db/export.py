...
from .models import (
    ...
    ClimateData, CellCulture, CellReligion, ReligionCulture
)
...
class PostGISExporter:
    ...
    def _export_map_metadata(self, map_name: str, metadata: Dict[str, Any]) -> uuid.UUID:
    ...
    def _export_settlements(
        ...
        for settlement_id, settlement in settlements_data.items():
            ...
            if hasattr(voronoi_graph, 'points') and cell_index < len(voronoi_graph.points):
    ...
    def _export_climate_data(
        ...
        if hasattr(climate_data, 'temperature') and hasattr(climate_data, 'precipitation'):
            ...
            for cell_idx in range(len(voronoi_graph.points)):
                if cell_idx < len(temp_data) and cell_idx < len(precip_data):
                    climate_record = ClimateData(
                        ...
                        altitude_m=int(voronoi_graph.heights[cell_idx]) if hasattr(voronoi_graph, 'heights') else 0
                    )
        ...
        for culture_id, culture in cultures_data.items():
            ...
            for cell_idx in cells:
                assignment = CellCulture(
                    ...
                    population=float(getattr(culture, 'population', 0)) / len(cells) if cells else 0
                )
    ...
    def _create_territory_geometry(
        ...
        try:
            ...
            logger.warning("Failed to create territory geometry", 
                         entity_id=entity_id, entity_type=entity_type, error=str(e))
            return None
