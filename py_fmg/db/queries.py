...
from .models import (
    Map, Culture, Religion, State, Settlement, River, BiomeRegion,
    ClimateData, CellCulture, CellReligion
)
...
class PostGISQueries:
    ...
    def list_maps(self, limit: int = 50) -> List[Map]:
        ...
        return self.session.query(Map).order_by(Map.created_at.desc()).limit(limit).all()
    ...
    def get_territories_at_point(
        ...
        try:
            # Query culture
            culture = self.session.query(Culture).filter(
                and_(
            ...
            # Query religion
            religion = self.session.query(Religion).filter(
                and_(
            ...
            state = self.session.query(State).filter(
                and_(
                    State.map_id == map_id,
                    func.ST_Contains(State.geometry, point)
                )
            ).first()
            ...
            # Query biome
            biome = self.session.query(BiomeRegion).filter(
                and_(
    ...
    def get_climate_data(
        ...
        climate_query = self.session.query(
            ...
            and_(
                ...
                func.ST_DWithin(
                    func.ST_SetSRID(func.ST_Point(ClimateData.latitude, ClimateData.latitude), 4326),
                    point,
    ...
    def get_map_statistics(self, map_id: uuid.UUID) -> Dict[str, Any]:
        ...
        try:
            # Basic counts
            stats['cultures_count'] = self.session.query(Culture).filter(
                Culture.map_id == map_id
            ).count()
            
            stats['religions_count'] = self.session.query(Religion).filter(
                Religion.map_id == map_id
            ).count()
            
            stats['states_count'] = self.session.query(State).filter(
                State.map_id == map_id
            ).count()
            
            stats['settlements_count'] = self.session.query(Settlement).filter(
                Settlement.map_id == map_id
            ).count()
            
            stats['rivers_count'] = self.session.query(River).filter(
                River.map_id == map_id
            ).count()
            
            stats['biomes_count'] = self.session.query(BiomeRegion).filter(
                BiomeRegion.map_id == map_id
            ).count()
            
            # Population statistics
            total_pop = self.session.query(
                func.sum(Settlement.population)
            ).filter(Settlement.map_id == map_id).scalar()
            
            stats['total_population'] = int(total_pop) if total_pop else 0
            
            # Largest settlement
            largest_settlement = self.session.query(Settlement).filter(
                Settlement.map_id == map_id
            ).order_by(Settlement.population.desc()).first()
            ...
            # Territory areas
            total_land_area = self.session.query(
                func.sum(func.ST_Area(Culture.geometry))
            ).filter(Culture.map_id == map_id).scalar()
            
            stats['total_land_area_km2'] = float(total_land_area) if total_land_area else 0
