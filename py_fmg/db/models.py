...
class Map(Base):
    ...
    # Relationships
    voronoi_cells = relationship(
        "VoronoiCell", back_populates="map", cascade="all, delete-orphan"
    )
    cultures = relationship(
        "Culture", back_populates="map", cascade="all, delete-orphan"
    )
    religions = relationship(
        "Religion", back_populates="map", cascade="all, delete-orphan"
    )
    states = relationship("State", back_populates="map", cascade="all, delete-orphan")
    settlements = relationship(
        "Settlement", back_populates="map", cascade="all, delete-orphan"
    )
    rivers = relationship("River", back_populates="map", cascade="all, delete-orphan")
    biomes = relationship(
        "BiomeRegion", back_populates="map", cascade="all, delete-orphan"
    )

    climate_data = relationship(
        "ClimateData", back_populates="map", cascade="all, delete-orphan"
    )
    cell_cultures = relationship(
        "CellCulture", back_populates="map", cascade="all, delete-orphan"
    )
    cell_religions = relationship(
        "CellReligion", back_populates="map", cascade="all, delete-orphan"
    )
...
class Culture(Base):
    ...
    )  # Naval, Highland, River, Lake, Nomadic, Hunting, Generic
