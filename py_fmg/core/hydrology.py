...
class Hydrology:
    ...
    def simulate_water_flow(self):
        ...
        logger.info(
            "Water flow simulation completed",
    ...
    def _trace_river_path(
        self, start_cell: int, existing_rivers: Set[int]
    ) -> List[int]:
        """
        ...
        while True:
            ...
            ):  # Circular flow (shouldn't happen after depression filling)
                break
            if next_cell in existing_rivers:  # Reached another river
                path.append(next_cell)
                break
            if (
                self.water_flux[next_cell] < self.options.min_river_flow
            ):  # Flow too small
                break
    ...
    def detect_lakes(self):
        ...
        for i in range(len(self.graph.points)):
            ...
            if (
                self.water_flux[i] > self.options.min_river_flow * 0.5
                and self.flow_directions[i] == -1
            ):  # No outflow
        ...
        for candidate_cell in lake_candidates:
            ...
            if len(lake_cells) >= 3:  # Minimum size for a lake
                water_level = np.mean(
                    [self.filled_heights[cell] for cell in lake_cells]
                )
    ...
    def _find_lake_cells(self, start_cell: int, processed_cells: Set[int]) -> Set[int]:
        ...
        while queue:
            ...
            if (
                abs(self.filled_heights[cell] - start_height) < 2.0
                and self.water_flux[cell] > 0
            ):
        ...
        for cell in lake_cells:
            for neighbor in self.graph.cell_neighbors[cell]:
                if neighbor not in lake_cells:
                    ...
                    height_diff = (
                        self.filled_heights[cell] - self.filled_heights[neighbor]
                    )
    ...
    def run_full_simulation(self):
        ...
        logger.info(
            "Hydrology simulation completed",
    ...
    def alter_heights(self) -> None:
        ...
        for i in range(len(self.graph.heights)):
            if self.graph.heights[i] >= self.options.sea_level:
                ...
                if neighbors:
                    neighbor_distances = [
                        self.graph.distance_field[n]
                        for n in neighbors
                        if n < len(self.graph.distance_field)
                    ]
                    mean_neighbor_distance = (
                        np.mean(neighbor_distances) if neighbor_distances else 0
                    )
                else:
    ...
    def resolve_depressions(self) -> None:
        ...
        for i in range(len(self.graph.heights)):
            if (
                self.graph.heights[i] >= self.options.sea_level
                and not self.graph.cell_border_flags[i]
            ):
                land.append(i)
        ...
        for iteration in range(max_iterations):
            ...
            if iteration < check_lake_max_iteration:
                for lake in lakes:
                    ...
                    lake_height = (
                        lake.height
                        if hasattr(lake, "height") and lake.height is not None
                        else 0
                    )
            ...
            if depressions == 0:
                logger.info(
                    f"Depression resolution converged after {iteration + 1} iterations"
                )
                break

        if depressions > 0:
            logger.warning(
                f"Unresolved depressions: {depressions}. Edit heightmap to fix"
            )
                        ...
                        if lake_cell is not None:
                            ...
                            if self.river_ids[lake_cell] != 0:
                                ...
                                if not same_river:
                                    ...
                                    self.rivers[self.next_river_id].cells.append(
                                        lake_cell
                                    )
    ...
    def _find_flow_target_excluding_lakes(
        self, cell_id: int, lake_ids: List[int]
    ) -> Optional[int]:
                        ...
                        for feature in self.features.features:
                            if (
                                ...
                                if hasattr(feature, "type") and feature.type == "lake":
                                    ...
                                    if not hasattr(
                                        feature, "river"
                                    ) or from_flux > getattr(
                                        feature, "enteringFlux", 0
                                    ):
                                        feature.river = river_id
                                        feature.enteringFlux = from_flux
                                    feature.flux = (
                                        getattr(feature, "flux", 0) + from_flux
                                    )
    ...
    def _calculate_river_width(
        self, discharge: float, river_cells: Optional[List[int]] = None
    ) -> float:
        """
        ...
        if river_cells and len(river_cells) >= 2:
            slope = max(
                self._calculate_average_slope(river_cells), self.options.min_slope
            )
