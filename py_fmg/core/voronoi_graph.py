...
class VoronoiGraph:
    ...
    distance_field: Optional[np.ndarray] = field(
        default=None
    )  # cells.t - distance to coast
    feature_ids: Optional[np.ndarray] = field(
        default=None
    )  # cells.f - feature ID for each cell
    ...
    def should_regenerate(self, config: GridConfig, seed: str) -> bool:
        ...
        """
        same_size = (
            self.graph_width == config.width and self.graph_height == config.height
        )
...
def get_jittered_grid(
    width: float, height: float, spacing: float, seed: str = None
) -> np.ndarray:
    """
...
def generate_or_reuse_grid(
    existing_grid: Optional[VoronoiGraph],
    config: GridConfig,
    seed: str = None,
...
def generate_voronoi_graph(
    config: GridConfig, seed: str = None, apply_relaxation: bool = True
) -> VoronoiGraph:
    """
    ...
    logger.info(
        "Grid parameters calculated", spacing=spacing, cells_x=cells_x, cells_y=cells_y
    )
    ...
    if apply_relaxation:
        grid_points = relax_points(
            grid_points, boundary_points, config.width, config.height, n_iterations=3
        )
