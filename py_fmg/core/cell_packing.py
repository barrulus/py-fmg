...
def regraph(graph: VoronoiGraph) -> VoronoiGraph:
    ...
    if graph.distance_field is None:
        raise ValueError(
            "graph.distance_field not found. Call Features.markup_grid() first!"
        )
    ...
    logger.info(
        ...
        coastal_points_added=n_packed
        - np.sum(new_grid_indices == np.arange(len(new_grid_indices))),
    )
