...
class Settlements:
    ...
    def __init__(
        ...
        self.cell_religions = cell_religions if cell_religions is not None else np.zeros(len(graph.points), dtype=np.uint16)

        # Initialize arrays
        # Population and suitability are now calculated in cultures module
        self.cell_suitability = getattr(graph, 'cell_suitability', np.zeros(len(graph.points), dtype=np.int16))
        self.cell_population = getattr(graph, 'cell_population', np.zeros(len(graph.points), dtype=np.float32))
    ...
    def rank_cells(self) -> None:
        ...
        """
        logger.warning("rank_cells() called but population is now calculated in cultures module")
        return
    ...
    def place_towns(self) -> None:
        ...
        spacing = (self.graph.graph_width + self.graph.graph_height) / self.options.town_spacing_base
        if towns_number > 0:
    ...
    def _calculate_expansion_cost(
        ...
        if cell_culture == state_culture:
            ...
            if state_type in ["Highland", "Nomadic"]:
                culture_bonus *= 1.5  # Highland and nomadic cultures are more culturally cohesive
            cost += culture_bonus
        else:
        ...
        if self.graph.heights[cell_id] < 20:
            ...
            if state_type == "River" and hasattr(self.graph, 'river_ids') and self.graph.river_ids[cell_id] > 0:
                pop_cost *= 0.5  # River states prefer river cells even if less populated
            cost += pop_cost
        else:
        ...
        if hasattr(state, 'center_cell'):
            ...
            if state_type == "Nomadic":
                ...
                distance_penalty = distance / 80   # Naval states can project power via sea
            else:
    ...
    def _adjust_cultural_boundaries(self) -> None:
        ...
        for i in range(len(self.cell_state)):
            ...
            current_culture = self.cultures.cell_cultures[i] if hasattr(self.cultures, 'cell_cultures') else 0
            ...
            for n in land_neighbors:
                ...
                n_culture = self.cultures.cell_cultures[n] if hasattr(self.cultures, 'cell_cultures') else 0
                
                if n_culture == current_culture and n_culture != 0:  # Same culture
                    cultural_alignment[n_state_id] = cultural_alignment.get(n_state_id, 0) + 3
                elif n_state.culture_id == current_culture and current_culture != 0:  # Same state culture
                    cultural_alignment[n_state_id] = cultural_alignment.get(n_state_id, 0) + 2
                elif current_state.culture_id == n_culture and n_culture != 0:  # Cell culture matches current state
                    cultural_alignment[n_state_id] = cultural_alignment.get(n_state_id, 0) + 1
                else:  # No cultural alignment
                    cultural_alignment[n_state_id] = cultural_alignment.get(n_state_id, 0) + 0
            
            # Check if we should switch to a culturally better-aligned neighbor
            if neighbor_states:
                best_state = max(neighbor_states.keys(), 
                               key=lambda s: (cultural_alignment.get(s, 0), neighbor_states[s]))
                best_alignment = cultural_alignment.get(best_state, 0)
                current_alignment = 1 if current_state.culture_id == current_culture else 0
                
                # Switch if significantly better cultural alignment
                if (best_alignment > current_alignment + 1 and 
                    neighbor_states[best_state] >= 2):
    ...
    def _smooth_geographic_boundaries(self) -> None:
        ...
        for i in range(len(self.cell_state)):
            ...
            has_river = (hasattr(self.graph, 'river_ids') and 
                        i < len(self.graph.river_ids) and 
                        self.graph.river_ids[i] > 0)
            ...
            for n in neighbors:
                ...
                if has_river:
                    ...
                    if n_has_river:
                        ...
                        if (n_state_obj and current_state_obj and
                            n_state_obj.type == "River" and current_state_obj.type == "River"):
    ...
    def specify_settlements(self) -> None:
        ...
        for settlement in self.settlements.values():
            ...
            settlement.name = self.name_generator.generate_burg_name(settlement.culture_id)
    ...
    def _get_port_position(self, settlement: Settlement) -> Tuple[float, float]:
        ...
        try:
            ...
            if cell_id < len(distance_field) and distance_field[cell_id] == 1:  # LAND_COAST
                    ...
                    for dx in range(-radius, radius + 1):
                        for dy in range(-radius, radius + 1):
                            ...
                            if hasattr(self.graph, 'cells_x'):
                                ...
                                if (0 <= new_row < (len(self.graph.points) // cells_x) and 
                                    0 <= new_col < cells_x):
                                    test_id = new_row * cells_x + new_col
                                    
                                    if (test_id < len(self.graph.heights) and 
                                        self.graph.heights[test_id] < 20):
    ...
    def _get_settlement_type(self, settlement: Settlement) -> str:
        ...
        if self.graph.heights[cell_id] > 60:
            if culture_type == "Highland":
                return "Highland Fortress"  # Highland cultures build fortified settlements
            else:
                return "Highland"
        ...
        if culture_type == "Naval":
            # Check if near coast
            if (hasattr(self.graph, 'distance_field') and 
                cell_id < len(self.graph.distance_field) and
                self.graph.distance_field[cell_id] <= 2):  # Close to coast
                return "Coastal"
    ...
    def _assign_settlement_features(self, settlement: Settlement) -> None:
        ...
        base_citadel_chance = (
            1.0 if settlement.is_capital else
            0.75 if pop > 50 else
            0.5 if pop > 15 else
            0.1
        )
        ...
        base_shanty_chance = (
            1.0 if pop > 60 else
            0.75 if pop > 40 else
            0.4 if (pop > 20 and settlement.walls) else
            0.0
        )
        ...
        if settlement.culture_id in self.cultures.cultures:
            ...
            if culture_type == "Highland":
                ...
                religion_modifier -= 0.1  # Nomadic cultures have fewer permanent temples
        
        final_temple_chance = min(0.95, max(0.05, base_temple_chance + religion_modifier))
