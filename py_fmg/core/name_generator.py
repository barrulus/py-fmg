...
class NameGenerator:
    ...
    def generate_culture_short(self, culture_id: int, base_id: Optional[int] = None) -> str:
    ...
    def generate_state_name(
        ...
        if base == 5:  # Ruthenian
            if name.endswith(("sk", "ev", "ov")):
                name = name[:-2]
        elif base == 12:  # Japanese (not in current bases, but keeping for compatibility)
    ...
    def generate_river_name(self, culture_id: int, base_id: Optional[int] = None) -> str:
    ...
    def generate_map_title(self) -> str:
        ...
        base_id = self.prng.choice(list(self.name_bases.keys())) if self.name_bases else 0
