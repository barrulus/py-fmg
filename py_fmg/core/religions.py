...
class ReligionGenerator:
    ...
    FOLK_FORMS = ["Shamanism", "Animism", "Polytheism", "Ancestor Worship",
                  "Nature Worship", "Totemism"]
    ORGANIZED_FORMS = ["Polytheism", "Monotheism", "Dualism", "Pantheism", "Non-theism"]
    CULT_FORMS = ["Cult", "Dark Cult", "Sect"]

    # Theocracy state names
    THEOCRACY_FORMS = ["Diocese", "Bishopric", "Eparchy", "Exarchate",
                      "Patriarchate", "Imamah", "Caliphate"]
    ...
    def __init__(
        ...
        self.religion_colors = [
            ...
            "#ffa500", "#ffc0cb", "#a0522d", "#8b4513", "#dda0dd", "#98fb98"
        ]
    ...
    def _generate_organized_religions(self) -> None:
        ...
        religions_to_generate = min(self.options.religions_number, len(potential_centers))
        ...
        for i in range(religions_to_generate):
            ...
            religion.name = self._generate_organized_religion_name(religion, parent_culture)
            religion.code = self._generate_religion_code(religion)
    ...
    def _find_religion_centers(self) -> List[int]:
        ...
        for cell_id in range(len(self.graph.points)):
            if (cell_id < len(self.graph.heights) and
                self.graph.heights[cell_id] >= 20):  # Land only
    ...
    def _expand_organized_religions(self) -> None:
        ...
        organized_religions = [r for r in self.religions.values()
                             if r.type in ["Organized", "Cult"]]
    ...
    def _add_neighbors_to_expansion_queue(
        ...
        for neighbor_id in self.graph.neighbors[cell_id]:
            if (neighbor_id < len(self.graph.heights) and
                self.graph.heights[neighbor_id] >= 20):  # Land only
    ...
    def _calculate_expansion_cost(
        ...
        # Culture cost
        from_culture = self.cell_cultures[from_cell] if from_cell < len(self.cell_cultures) else 0
        to_culture = self.cell_cultures[to_cell] if to_cell < len(self.cell_cultures) else 0
    ...
    def _can_expand_to_cell(self, religion: Religion, cell_id: int) -> bool:
        ...
        if religion.expansion == "culture":
            cell_culture = self.cell_cultures[cell_id] if cell_id < len(self.cell_cultures) else 0
    ...
    def _calculate_statistics(self) -> None:
        ...
        for religion in self.religions.values():
            ...
            for cell_id in religion.cells:
                if hasattr(self.graph, 'cell_population') and cell_id < len(self.graph.cell_population):
                    population = self.graph.cell_population[cell_id]
    ...
    def _generate_deity_meaning(self) -> str:
        ...
        approaches = []
        approach_weights = {
            "Number": 1, "Being": 3, "Adjective": 5,
            "Color + Animal": 5, "Adjective + Animal": 5, "Adjective + Being": 5,
            "Adjective + Genitive": 1, "Color + Being": 3, "Color + Genitive": 3,
            "Being + of + Genitive": 2, "Being + of the + Genitive": 1,
            "Animal + of + Genitive": 1, "Adjective + Being + of + Genitive": 2,
        ...
        base_terms = {
            ...
            "being": [
                "Ancestor", "Ancient", "Avatar", "Brother", "Champion", "Chief", "Council", "Creator",
                "Deity", "Divine One", "Elder", "Enlightened Being", "Father", "Forebear", "Forefather",
                "Giver", "God", "Goddess", "Guardian", "Guide", "Hierach", "Lady", "Lord", "Maker",
                "Master", "Mother", "Numen", "Oracle", "Overlord", "Protector", "Reaper", "Ruler",
                "Sage", "Seer", "Sister", "Spirit", "Supreme Being", "Transcendent", "Virgin"
            ],
            "animal": [
                "Antelope", "Ape", "Badger", "Basilisk", "Bear", "Beaver", "Bison", "Boar", "Buffalo",
                "Camel", "Cat", "Centaur", "Cerberus", "Chimera", "Cobra", "Cockatrice", "Crane",
                "Crocodile", "Crow", "Cyclope", "Deer", "Dog", "Direwolf", "Drake", "Dragon", "Eagle",
                "Elephant", "Elk", "Falcon", "Fox", "Goat", "Goose", "Gorgon", "Gryphon", "Hare",
                "Hawk", "Heron", "Hippogriff", "Horse", "Hound", "Hyena", "Ibis", "Jackal", "Jaguar",
                "Kitsune", "Kraken", "Lark", "Leopard", "Lion", "Manticore", "Mantis", "Marten",
                "Minotaur", "Moose", "Mule", "Narwhal", "Owl", "Ox", "Panther", "Pegasus", "Phoenix",
                "Python", "Rat", "Raven", "Roc", "Rook", "Scorpion", "Serpent", "Shark", "Sheep",
                "Snake", "Sphinx", "Spider", "Swan", "Tiger", "Turtle", "Unicorn", "Viper", "Vulture",
                "Walrus", "Wolf", "Wolverine", "Worm", "Wyvern", "Yeti"
            ],
            "adjective": [
                "Aggressive", "Almighty", "Ancient", "Beautiful", "Benevolent", "Big", "Blind", "Blond",
                "Bloody", "Brave", "Broken", "Brutal", "Burning", "Calm", "Celestial", "Cheerful",
                "Crazy", "Cruel", "Dead", "Deadly", "Devastating", "Distant", "Disturbing", "Divine",
                "Dying", "Eternal", "Ethernal", "Empyreal", "Enigmatic", "Enlightened", "Evil",
                "Explicit", "Fair", "Far", "Fat", "Fatal", "Favorable", "Flying", "Friendly", "Frozen",
                "Giant", "Good", "Grateful", "Great", "Happy", "High", "Holy", "Honest", "Huge",
                "Hungry", "Illustrious", "Immutable", "Ineffable", "Infallible", "Inherent", "Last",
                "Latter", "Lost", "Loud", "Lucky", "Mad", "Magical", "Main", "Major", "Marine",
                "Mythical", "Mystical", "Naval", "New", "Noble", "Old", "Otherworldly", "Patient",
                "Peaceful", "Pregnant", "Prime", "Proud", "Pure", "Radiant", "Resplendent", "Sacred",
                "Sacrosanct", "Sad", "Scary", "Secret", "Selected", "Serene", "Severe", "Silent",
                "Sleeping", "Slumbering", "Sovereign", "Strong", "Sunny", "Superior", "Supernatural",
                "Sustainable", "Transcendent", "Transcendental", "Troubled", "Unearthly", "Unfathomable",
                "Unhappy", "Unknown", "Unseen", "Waking", "Wild", "Wise", "Worried", "Young"
            ],
            "genitive": [
                "Cold", "Day", "Death", "Doom", "Fate", "Fire", "Fog", "Frost", "Gates", "Heaven",
                "Home", "Ice", "Justice", "Life", "Light", "Lightning", "Love", "Nature", "Night",
                "Pain", "Snow", "Springs", "Summer", "Thunder", "Time", "Victory", "War", "Winter"
            ],
            "the_genitive": [
                "Abyss", "Blood", "Dawn", "Earth", "East", "Eclipse", "Fall", "Harvest", "Moon",
                "North", "Peak", "Rainbow", "Sea", "Sky", "South", "Stars", "Storm", "Sun", "Tree",
                "Underworld", "West", "Wild", "Word", "World"
            ],
            "color": [
                "Amber", "Black", "Blue", "Bright", "Bronze", "Brown", "Coral", "Crimson", "Dark",
                "Emerald", "Golden", "Green", "Grey", "Indigo", "Lavender", "Light", "Magenta",
                "Maroon", "Orange", "Pink", "Plum", "Purple", "Red", "Ruby", "Sapphire", "Teal",
    ...
    def assign_temples_to_settlements(self, settlements_dict: Dict) -> None:
        ...
        for settlement in settlements_dict.values():
            ...
            if hasattr(settlement, 'state_id') and settlement.state_id in self.states_dict:
                state = self.states_dict[settlement.state_id]
                theocracy_bonus = hasattr(state, 'type') and state.type in self.THEOCRACY_FORMS
