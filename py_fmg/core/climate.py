...
class ClimateOptions:
    ...
    temperature_equator: float = 25  # °C at equator
    temperature_north_pole: float = -30  # °C at north pole
    temperature_south_pole: float = -30  # °C at south pole
...
class Climate:
    ...
    def calculate_temperatures(self):
        ...
        tropics = [16, -20]  # Northern and southern tropic latitudes
        tropical_gradient = 0.15
    ...
    def generate_precipitation(self):
        ...
        # Latitude modifiers for precipitation
        # Based on atmospheric circulation cells (Hadley, Ferrel, Polar)
        latitude_modifier = [
            4,    # 0-5°: Wet all year (ITCZ)
            2, 2, # 5-20°: Wet summer, dry winter
            1, 1, # 20-30°: Dry all year (descending air)
            2, 2, # 30-40°: Wet winter, dry summer  
            2, 2, # 40-50°: Wet winter, dry summer
            3, 3, # 50-60°: Wet all year
            2, 2, # 60-70°: Wet summer, dry winter
            1, 1, # 70-80°: Dry all year
            1,    # 80-85°: Dry all year
            0.5   # 85-90°: Very dry
        ]
        ...
        for c in range(0, n_cells, cells_x):
            ...
            # Wind tier (0-5 from N to S)
            wind_tier = int(abs(lat - 89) / 30)
            wind_tier = min(wind_tier, 5)
        ...
        # Pass winds across the map
        if westerly:
            self._pass_wind(westerly, 120 * modifier, 1, cells_x)
        if easterly:
            self._pass_wind(easterly, 120 * modifier, -1, cells_x)
    ...
    def _pass_wind(self, source: List, max_prec: float, next_step: int, steps: int):
        ...
        for first in source:
            ...
            for s in range(steps):
                ...
                # Skip permafrost
                if hasattr(self.graph, 'temperatures') and self.temperatures[current] < -5:
                    current += next_step
                    continue
                ...
                if self.graph.heights[current] < 20:
                    ...
                    if 0 <= next_cell < len(self.graph.points):
                        if self.graph.heights[next_cell] >= 20:
                            ...
                            precip = max(humidity / np.random.randint(10, 21), 1)
                            self.precipitation[next_cell] += int(min(precip, 255 - self.precipitation[next_cell]))
                        else:
                            # Wind gains humidity over water
                            humidity = min(humidity + 5 * self.options.precipitation_modifier, max_prec)
                            water_precip = int(5 * self.options.precipitation_modifier)
                ...
                if 0 <= next_cell < len(self.graph.points):
                    ...
                    # Update humidity
                    evaporation = 1 if precipitation > 1.5 else 0
    ...
    def _get_precipitation(self, humidity: float, cell_idx: int, next_step: int) -> float:
        ...
        """
        # Normal precipitation loss
        normal_loss = max(humidity / (10 * self.options.precipitation_modifier), 1)
        ...
        if 0 <= next_idx < len(self.graph.points):
            ...
            terrain_mod = (self.graph.heights[next_idx] / 70) ** 2  # Mountains at 70+
