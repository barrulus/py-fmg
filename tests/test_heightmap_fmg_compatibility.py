...
class TestHeightmapFMGCompatibility:
    ...
    def test_heightmap_statistical_compatibility(self):
        ...
        python_heights = generator.from_template(template, seed=self.seed)
        ...
        print(f"  Python heights: shape={python_heights.shape}, "
              f"range=[{np.min(python_heights)}, {np.max(python_heights)}], "
              f"mean={np.mean(python_heights):.1f}")
        print(f"  FMG heights: shape={fmg_grid_heights.shape}, "
        ...
        assert 40 < python_land_pct < 95, f"Python land percentage {python_land_pct:.1f}% out of expected range"
        assert 40 < fmg_land_pct < 95, f"FMG land percentage {fmg_land_pct:.1f}% out of expected range"
    ...
    def test_height_statistics_match(self):
        ...
        # Compare statistics
        print("\nHeight statistics comparison:")
        print(f"  Python - min: {np.min(python_heights)}, max: {np.max(python_heights)}, "
              f"mean: {np.mean(python_heights):.1f}, std: {np.std(python_heights):.1f}")
        print(f"  FMG    - min: {np.min(fmg_grid_heights)}, max: {np.max(fmg_grid_heights)}, "
        ...
        print(f"  FMG land cells: {fmg_land} ({fmg_land/len(fmg_grid_heights)*100:.1f}%)")
...
def test_lowisland_template_characteristics():
    ...
    heights = generator.from_template(template, seed="lowisland_test")
