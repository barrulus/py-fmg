...
def client(test_db):
    ...
    with patch.dict(os.environ, {
        ...
        with patch('py_fmg.api.main.Map', TestMap), \
             patch('py_fmg.api.main.GenerationJob', TestGenerationJob):
...
def test_api_different_templates(client: TestClient, template_name: str) -> None:
    ...
    for _ in range(max_attempts):
        ...
        if status_data["status"] == "completed":
            ...
            pytest.fail(f"Template {template_name} generation failed: {status_data.get('error_message', 'Unknown error')}")
