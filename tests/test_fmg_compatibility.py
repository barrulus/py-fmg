...
class TestVoronoiTopologyCompatibility:
    ...
    def setup_class(cls):
        ...
        print(f"Loaded reference data: seed={cls.seed}, dimensions={cls.width}x{cls.height}, cells={cls.n_cells}")
    ...
    def test_point_coordinates_exact_match(self):
        ...
        assert len(python_graph.points) == len(fmg_points), \
    ...
    def test_vertex_connectivity_match(self):
        ...
        if mismatches:
            ...
            for mismatch in mismatches[:5]:
                print(f"Cell {mismatch['cell']}: Python={mismatch['python_count']} vertices, FMG={mismatch['fmg_count']} vertices")
...
class TestReferenceDataStructure:
    ...
    def test_cell_data_fields(self):
        ...
        optional_fields = ['h', 'f', 'fl', 'r', 'biome', 'pop', 'burg', 'state']  # For future tests
        
        for field in required_fields:
            assert field in first_cell, f"Required field '{field}' missing from cell data"
    ...
    def test_topology_sanity_checks(self):
        ...
        print(f"Found {asymmetric_count} asymmetric relationships ({asymmetric_rate:.2%})")
        
        # Some asymmetry is expected due to boundary conditions
        assert asymmetric_rate < 0.1, f"Too many asymmetric relationships: {asymmetric_rate:.2%}"
